      ...
      <div className="modal-body-premium" style={{ maxHeight: '65vh', overflowY: 'auto', paddingRight: '4px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '25px' }}>
          ...
          <div style={{ gridColumn: 'span 2', background: '#f8fafc', padding: '10px 14px', borderRadius: '6px', border: '1px solid #e2e8f0' }}>
            ...
            <span style={{ fontSize: '14px', color: '#1e293b', fontWeight: '500', display: 'block', whiteSpace: 'pre-wrap' }}>{orderData.alamat || 'Tidak ada alamat pengiriman tercatat.'}</span>
          </div>
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
          ...
          <tbody>
            {(orderData.details || []).length > 0 ? (
              orderData.details.map((item, index) => (
                <tr key={item.id || index} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  <td style={{ textAlign: 'left', padding: '10px 0' }}>
                    <strong>{item.barang ? item.barang.nmBarang : 'Barang Terhapus'}</strong>
                  </td>
              ...
              <tr>
                <td colSpan="4" style={{ textAlign: 'center', color: '#888', padding: '15px' }}>📭 Tidak ada rincian item ditemukan.</td>
              </tr>
            )}
          </tbody>
        </table>
